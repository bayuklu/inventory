...
const Outlet = () => {
  ...
  const [hideFormAddOutlet, setHideFormAddOutlet] =
    useState("hideFormAddProduct");
  const [hideFormEditOutlet, setHideFormEditOutlet] =
    useState("hideFormAddProduct");
  ...
  const getOutlet = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BASEURL}/outlet`
      );
  ...
  const handleDeleteOutlet = async (outletId, outletName) => {
    if (
      confirm(`Are you sure want to delete ${outletName.toUpperCase()}`) != true
    )
      return;
    try {
      const response = await axios.delete(
        `${import.meta.env.VITE_BASEURL}/outlet/${outletId}`
      );
      if (response) {
        setMsg({ msg: response.data.msg, color: "green" });
        setOutlets(response.data.data);
      ...
      <div className="outletContainer">
        <div className="outletHeader">
          ...
          <div className="addOutlet">
            ...
            <button onClick={handleShowFormAddOutlet} className="button">
              Add Outlet
            </button>
          </div>
        </div>
          ...
          {currentOutlets.map((outlet, index) => (
            <div key={index} className="outletBox">
              ...
              <div style={{ display: "flex", gap: "5px" }}>
                ...
                <h1 style={{ fontSize: "25px", color: "darkorange" }}>
                  {outlet.name}
                </h1>
              </div>
              <div
                style={{ display: "flex", gap: "5px", marginBottom: "55px" }}
              >
                <i>
              ...
              >
                ...
                <p>{outlet.phone}</p>
                <a
                  href={`https://wa.me/+62${outlet.phone}`}
                  target="_blank"
                  className="button"
                  style={{
                    height: "30px",
        ...
        {/* form add outlet */}
        <div
          onClick={handleHideFormAddOutlet}
          className={`formAddProduct ${hideFormAddOutlet}`}
        >
          <form
            style={{ height: "auto" }}
          ...
          >
            ...
            <div className="field">
              ...
              <p style={{ color: "white" }} className="help">
                Masukkan nomor hp pemilik outlet
              </p>
            </div>
        ...
        {/* form edit outlet */}
        <div
          onClick={handleHideFormEditOutlet}
          className={`formAddProduct ${hideFormEditOutlet}`}
        >
          <form
            style={{ height: "auto" }}
